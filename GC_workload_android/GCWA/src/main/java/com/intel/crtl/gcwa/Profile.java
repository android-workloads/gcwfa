...
public class Profile {
	...
	public class ProfileData {
	    ...
	    public void setLosElementDist(float[] los_element_dist) { mLosElementDist = los_element_dist; }
	    ...
	    public void setThreadNum(int thread_num) { mThreadNum = mThreadMode? 1 : thread_num; }
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                ...
                case XmlPullParser.START_TAG:
                    ...
                    if (name.matches("profile")) {
                        newProfile = new ProfileData(Integer.parseInt(parser.getAttributeValue(0)),
                        ...
                        if ("lifetime".equals(parser.getAttributeValue(0))) {
                            newProfile.mLifetime = new float[Integer.parseInt(parser.getAttributeValue(1))][];
                            idx0 = 0;
                        }
                    } else if(name.matches("float-array")) {
                        itemName = parser.getAttributeValue(0);
                        if (itemName.matches("size_dist")) {
                            newProfile.mSizeDist = new float[Integer.parseInt(parser.getAttributeValue(1))];
                            floatValues = newProfile.mSizeDist;
                        } else if (itemName.matches("los_element_dist")) {
                            newProfile.mLosElementDist = new float[Integer.parseInt(parser.getAttributeValue(1))];
                            floatValues = newProfile.mLosElementDist;
                        } else if (idx0 >= 0) {
                            newProfile.mLifetime[idx0] = new float[Integer.parseInt(parser.getAttributeValue(1))];
